//gather slides from images container 
let slides = document.querySelectorAll('#slides .slide');
// keeps count of current slide
let currentSlide = 0;  
// frequency of image change
let slideInterval = setInterval(nextSlide,5000);

        function nextSlide() {
            // changing class name of current slide. The transition is handled by CSS
            slides[currentSlide].className = 'slide'; 
            // using % to cycle back to 0 when the slides run through 
            currentSlide = (currentSlide+1)%slides.length;
            //when we have the new slides number, we give that slide a new class name then CSS makes the transition
